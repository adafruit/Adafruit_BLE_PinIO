...
/* Wrapper to read a single byte from BLE uart (returns -1 if buffer empty) */
int BLEFirmataClass::read()
{
  /* ToDo! */
  return -1;
}
...
BLEFirmataClass::BLEFirmataClass()
{
...
void BLEFirmataClass::begin(Adafruit_BLE_UART &ble_uart)
{
  uart = &ble_uart;
  uart->begin();
  delay(1000);
...
void BLEFirmataClass::printVersion(void) {
  ...
  write(FIRMATA_MAJOR_VERSION);
  write(FIRMATA_MINOR_VERSION);
}
...
// void BLEFirmataClass::unsetFirmwareVersion()
// {
//   firmwareVersionCount = 0;
//   free(firmwareVersionVector); 
//   firmwareVersionVector = 0;
...
{
  /* ToDo: Return a value if data is available in the buffer */
...
void BLEFirmataClass::write(byte c)
{
  uart->write(&c, 1);
}
 ...
 * this is too complicated for analogReceive, but maybe for Sysex?
 void BLEFirmataClass::attachSysex(sysexFunction newFunction)
 {
 byte i;
...
void BLEFirmataClass::strobeBlinkPin(int count, int onInterval, int offInterval) 
{
  byte i;
  pinMode(VERSION_BLINK_PIN, OUTPUT);
  for(i=0; i<count; i++) {
